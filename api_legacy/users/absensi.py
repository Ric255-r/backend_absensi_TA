...
import os
from api_legacy.admin.get_data import absensi_connection
...
LATITUDE_BENGKOM = -0.028758
LONGITUDE_BENGKOM = 109.3350368
    ...
    async with pool.acquire() as conn:
      async with conn.cursor(aiomysql.DictCursor) as cursor:
        try:
          ...
          jadwal_checkin_timedelta = schedule_item[
            "shift_mulai"
          ]  # Ini Jg. Jadi jam 8:00:00.
          ...
          if waktu_checkin_sekarang > batas_waktu_checkin:
            is_telat = 1
            print(
              f"Karyawan telat. Check-in: {waktu_checkin_sekarang.time()}, Batas Waktu: {batas_waktu_checkin.time()}"
            )
          else:
            is_telat = 0
            print(
              f"Karyawan tepat waktu. Check-in: {waktu_checkin_sekarang.time()}, Batas Waktu: {batas_waktu_checkin.time()}"
            )
    ...
    async with pool.acquire() as conn:
      async with conn.cursor(aiomysql.DictCursor) as cursor:
        try:
          ...
          """
          q1_values = (
            data["latitude_checkout"],
            data["longitude_checkout"],
            filename,
            user["id_karyawan"],
          )
          await cursor.execute(q1, q1_values)
