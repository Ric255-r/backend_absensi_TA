...
from app.core.subscription import (
  ...
  get_subscription_now,
  is_subscription_usable,
  normalize_subscription_datetime,
)
...
def _parse_datetime(value: str, field_name: str) -> datetime:
  try:
    return normalize_subscription_datetime(
...
def _serialize_subscription(subscription: Subscription | None) -> dict | None:
  ...
  end_at = normalize_subscription_datetime(subscription.end_at)
  expired_at = (
    normalize_subscription_datetime(subscription.expired_at)
    if subscription.expired_at
    else None
  )
  grace_end_at = get_subscription_grace_end(end_at)
  is_in_grace = end_at < now <= grace_end_at
  ...
  return {
    ...
    "is_usable": is_usable,
    "days_remaining": max((end_at.date() - now.date()).days, 0),
...
async def expire_subscription_job() -> dict:
  ...
  subscription = await get_effective_subscription(now=get_subscription_now())
...
async def create_subscription(
  ...
  if end_at <= start_at:
    raise HTTPException(
      status_code=400, detail="end_at harus lebih besar dari start_at"
    )
...
async def extend_subscription(
  ...
  if new_end_at <= normalize_subscription_datetime(subscription.end_at):
    raise HTTPException(
      status_code=400,
